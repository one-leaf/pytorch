...
# 从文件中读取句子进行分割，并按给定的fields转为Dict
...
# 从对话标记文件 movie_conversations.txt 中读取对话的标记，用fields匹配，并加载前面 lines 的对话内容
...
# 按对话标记生成一对对的问答句子，其中忽略最后一句的问答和空行
def extractSentencePairs(conversations):
    qa_pairs = []
    for conversation in conversations:
        for i in range(len(conversation["lines"]) - 1):  # 忽略最后一行，因为这一行没有回答
...
######################################################################
# 创建一对对的问答样本文件 formatted_movie_lines.txt ，其中每一行表示一个完整对话，按tab分割
#
...
# 定义栏位 
# MOVIE_LINES_FIELDS lineID:行号 characterID:发言人ID movieID：电影ID character：发言人 text：内容
# MOVIE_CONVERSATIONS_FIELDS character1ID 第一个发言人ID character2ID 第二个发言人ID movieID 电影ID utteranceIDs 包含的句子
lines = {}
conversations = []
...
class Attn(nn.Module):
    def __init__(self, method, hidden_size):
        ...
        if self.method == 'general':
            # 全连接层，输入和输出都是 hidden_size
            self.attn = nn.Linear(self.hidden_size, hidden_size)
        elif self.method == 'concat':
            # 全连接层，输入是2倍的 hidden_size 

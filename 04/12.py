...
# 由于 reverse=True，从而 input是法语， output是英语
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        output = self.embedding(input).view(1, 1, -1)           # [1, 1, 256]
        output = F.relu(output)
        output, hidden = self.gru(output, hidden)               # [1, 1, 256] [1, 1, 256]
        output = self.softmax(self.out(output[0]))              # [1, 2803]
        return output, hidden
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # 直接将隐藏层和当前词向量合并对整个句子最大长度求归一化权重 [1, 10]
        attn_weights = F.softmax(
            self.attn(torch.cat((embedded[0], hidden[0]), 1)), dim=1)     

        # 将权重赋予编码器输出 [1, 1, 10] [1, 10, 256]  => [1, 1, 256]
        attn_applied = torch.bmm(attn_weights.unsqueeze(0),
                                 encoder_outputs.unsqueeze(0))               

        # 将解码器的输出和当前词向量合并 [1, 256] [1 256] => [1, 256*2]
        output = torch.cat((embedded[0], attn_applied[0]), 1)
        # 全连接到 [1, 256*2] => [1, 256]
        output = self.attn_combine(output).unsqueeze(0)

        # 将要原始要翻译的信息和当前的词向量，同时混合上一个词汇的信息，丢进RNN学习
        output = F.relu(output)
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    # 原始待翻译的编码信息 [句子最大长度，隐藏层长度] 这里隐藏层等于词向量的长度
...
if os.path.exists(loadFilename):
    ...
    torch.save({    'en': encoder1.state_dict(),

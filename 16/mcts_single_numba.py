...
class MCTS():
    ...
    def get_action_probs(self, state:State, temp:float=1):
        ...
        for n in range(self._n_playout):
            ...
            if (min(self.Qsa[s])>=0 and n>=7) or (depth>50 and self.Ns[s]>200) :
                # 如果当前都为正数，就不再搜索了
                ig_probs = True
                break
    ...
    def search(self, state:State):
        ...
        if state.game.terminal: 
            return -1-1/(1+state.game.removedlines)  # 游戏结束，返回-1，奖励越多，分数越高   
        s = hash(state)
...
class MCTSPlayer(object):
    ...
    def get_action(self, game, temp=0):        
        ...
        if not game.terminal:  # 如果游戏没有结束
            ...
            if idx == -1 or min(act_qs)>=0 or game.removedlines > self.min_score:

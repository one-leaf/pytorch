...
class Train():
    ...
    def run(self):
        ...
        try:
            ...
            begin_act_probs_e = np.exp(begin_act_probs-np.max(begin_act_probs))+1e-40
            begin_act_probs = begin_act_probs_e/np.sum(begin_act_probs_e, axis=1, keepdims=True)
            end_act_probs_e = np.exp(end_act_probs-np.max(end_act_probs))+1e-40
            ...
            if np.isnan(kl):
                ...
                print(np.sum(begin_act_probs_e, axis=1, keepdims=True))

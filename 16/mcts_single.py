...
class MCTS():
    ...
    def get_action_probs(self, game, temp=1, avg_ns=0):
        ...
        for n in count():
            ...
            if n >= self._n_playout/16-1 and game.piececount>game.score*2.5+game.must_reward_piece_count: break
    ...
    def search(self, game):
        ...
        if game.state==1 and game.piececount>game.prev_piececount+1: 
            reward = 0
            if game.reward>0:                
                reward = game.reward if game.failtop<=game.prev_failtop else -game.failtop/20

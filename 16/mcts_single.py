...
class MCTS():
    ...
    def get_action_probs(self, game, temp=1, avg_ns=0):
        ...
        if game.show_mcts_process or game.state == 1 :
            ...
            print(game.steps, game.fallpiece["shape"], "temp:", round(temp,2), "ns:", ns, "/", test_count, "depth:", self.max_depth, "empty:", game.getEmptyCount(), "height:", round(game.pieceheight,2), "value:", round(v,2), info, "diff:", round(game.getHeightDiff(),4))
...
class MCTSPlayer(object):
    ...
    def get_action(self, game, curr_player, temp=0, avg_ns=0, avg_piececount=0):        
        ...
        if not game.terminal:  # 如果游戏没有结束
            ...
            # 尝试其他的走法
            if random.random()<(game.piececount - avg_piececount)/(avg_piececount+1):
                p = 0.75

...
class Train():
    def __init__(self):
        ...
        self.n_playout = 1300  # 每个动作的模拟战记录个数，影响后续 512/2 = 256；256/16 = 16个方块 的走法
        # 64/128/256/512 都不行
        # step -> score
        # 128  --> 0.7
    ...
    def get_equi_data(self, states, mcts_probs, values, scores):
        ...
        for i in range(len(states)):
            ...
            if values[i]>0 and mcts_prob[0]<0.1:
        ...
        for playcount in range(3):
            ...
            for m in range(step_count-2):
                ...
                if m>0:# and data["steps"][m-1]["qval"]!=0:
                    ...
                    step_rewards[m]=((data["steps"][m+1]["qval"]-data["steps"][m-1]["qval"]))/3.

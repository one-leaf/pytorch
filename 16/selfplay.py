...
class Train():
    ...
    def get_equi_data(self, states, mcts_probs, model_probs, values, advs, actions, masks):
        """
        通过翻转增加数据集
        ...
        for i in range(len(states)):
            state, mcts_prob, model_prob, value, adv, action, mask = states[i], mcts_probs[i], model_probs[i], values[i], advs[i], actions[i], masks[i]
            extend_data.append((state, mcts_prob, model_prob, value, adv, action, mask))
            if mcts_prob[0]<0.2 and np.max(mcts_prob)>0.8: # 如果旋转的概率的不大，就做翻转
                ...
                if action in [1,2]: 
                    ...
                    equi_action = action
                extend_data.append((equi_state, equi_mcts_prob, equi_model_prob, value, adv, equi_action, mask))
            # if i==0:
            #     print("state:",state)
    ...
    def play(self, cache, state, sample_count, his_pieces, his_pieces_len, player, policy_value_net, max_steps):
        ...
        for i in count():
            ...
            dirichlet_p = 0.01 + min(0.9, i/(60*60*5))
    ...
    def collect_selfplay_data(self):
        ...
        if os.path.exists(model_file):
            if time.time()-os.path.getmtime(model_file)>60*60*5:
        ...
        player = MCTSPlayer(policy_value_net.policy_value_fn, c_puct=self.c_puct, n_playout=self.n_playout, limit_count=self.sample_count, piececount=state["total"]["piececount"])
        ...
        for i in range(self.play_count):
            probs_list.extend([play_data[i]["data"]["steps"][j]["mcts_probs"] for j in range(len(play_data[i]["data"]["steps"]))])
        ...
        if state["total"]["_agent"]>update_agent_count:
            ...
            state["update"].append(current_hour+current_day/100)
            ...
            # 如果当前最佳，将模型设置为最佳模型
            if max(state["steps"])==state["steps"][-1]:
                newmodelfile = model_file+"_steps_"+str(state["steps"][-1])
                if not os.path.exists(newmodelfile):
                    policy_value_net.save_model(newmodelfile)
        ...
        states, mcts_probs, model_probs, values, advs, actions, masks = [], [], [], [], [], [], []
        ...
        set_status_total_value(state, "mcts_probs", np.average(mcts_probs, axis=0), alpha)
        ...
        for i, obj in enumerate(self.get_equi_data(states, mcts_probs, model_probs, values, advs, actions, masks)):

...
class FiveChessEnv(gym.Env):
    ...
    def check_terminal(self):
        ...
        for x in range(self.SIZE):
            for y in range(self.SIZE):
                ...
                if x>=n and y>=n and abs(sum([self.chessboard[x-i][y-i] for i in range(n)]))==n:  
                    return True, self.players.index(color)
        return False, -1        
    ...
    def step(self, action):
        ...
        return self.chessboard,reward,terminal,user
...
if __name__ == "__main__":
    env = FiveChessEnv(size=10,n_in_row=3)
    while True:
        env.reset()
        done = False
        while not done:
            ...
            _, reward, done, user = env.step(action)
            env.render(True)

            if done:
                curr_user = env.get_current_player()
                step_count = env.step_count
                print(f'win_user: {user}, curr_user: {curr_user} reward: {reward} step_count: {step_count}')
                time.sleep(30)
                break

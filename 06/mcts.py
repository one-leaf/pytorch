...
class MCTS(object):
    ...
    def get_action_probs(self, state, temp=1e-3):
        ...
        print("player:",(state.step_count%2),"_n_playout:", n, "info:", info, "first:",self._first_ations)
        ...
        if len(state.availables) > 0:  # 盘面可落子位置>0
            ...
            if self._is_selfplay:  # 自我对抗
                ...
                if max(act_probs)>0.99:
                    ...
                    p = 1.0 - len(state.availables)/(state.size * state.size)*0.10  #【0.99~1】 这里默认为 0.25

...
class MCTS():
    ...
    def get_action_probs(self, state, temp=1):
        ...
        print(state.steps, state.curr_player, state.piecesteps, state.piececount, "n:", n, "depth:" ,self.max_depth, "value:", round(v,2), info, state.player_reward)
    ...
    def search(self, state):
        ...
        if s not in self.Es:
            ...
            # 这里增加了最终的奖励，提升对步骤的优化
            if end:
                if state.curr_player==state.winner: 
                    v = 1
                else:
                    v = -1 
                # v = 1
